...
def info_sarea(name:str):
    ...
    with conn:
        with conn.cursor() as cursor:
          #取出各區最新資料
            sql = '''
            SELECT 日期,站點資訊.站點名稱,行政區,站點地址,lat,lng,總車輪,可借,可還,活動
            FROM youbike
...
data= [tuple[0] for tuple in get_sarea()]
...
data1 = [{'日期':item[0],'站點':item[1],'總車輪':item[6],'可借':item[7],'可還':item[8],'活動':item[9]} for item in data]
#print(data1)
st.dataframe(data1)